...
**설정 단계 스코프 격리:** 설정 단계는 함수 내부와 동일한 격리된 지역 스코프에서 실행됩니다. 설정 중 생성된 변수는 바깥 스코프로 유출되지 않습니다. 전역 변수 접근에는 `::` 접두사가 필요합니다. 참고: `$var` 문법은 `$::var`를 지원하지 않으며, 동적 키에서 전역 접근에는 `$(::var)`를 사용하세요.
