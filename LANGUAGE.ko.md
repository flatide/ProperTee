...
**규칙:**
- 함수 내부: 일반 `x`는 지역 전용. 전역 접근에는 `::x` 사용.
- multi 블록 함수 내부: `::x`는 전역 스냅샷에서 읽음. `::x = value`는 런타임 에러 (스레드 순수성).
- 내장 프로퍼티 (호스트가 `-p`로 주입): 함수 내부에서 `::` 필요.
...
객체 키는 일반 식별자, 따옴표 문자열, 정수(문자열 키로 저장)를 사용할 수 있습니다.
...
| 변수 키 | `obj.$varName` | 변수에 저장된 키 이름 |
| 계산 키 | `obj.$(expression)` | 표현식으로 결정되는 키 |
| 숫자 키 | `obj.1` | 삽입 순서에 따른 위치 접근 (1-기반). 배열은 인덱스로 요소 접근. 객체는 N번째 항목 접근. |
...
설정 단계에서 루프를 지원하여 동적 스레드 생성이 가능합니다:
...
1. multi 블록 본문은 설정 단계로 실행되어 `thread` 호출을 수집
